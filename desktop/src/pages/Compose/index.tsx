      ...
      <div className="flex-1 relative overflow-hidden">
        {track.clips.map((clip) => (
          ...
          >
            ...
            <span className="text-[10px] text-white/80 truncate pl-1.5 leading-tight">{clip.label}</span>
          </button>
        ))}
      </div>
    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      className="h-8 rounded-md px-2 text-xs bg-[#27272a] border border-[#3f3f46] text-gray-200"
                    >
                      {VOICES.map(v => (
                        <option key={v.id} value={v.id} className="bg-[#27272a] text-gray-200">{v.name}</option>
                      ))}
                    </select>
                  </div>
