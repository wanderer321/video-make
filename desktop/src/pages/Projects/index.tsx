...
function ProjectCard({ project, onDelete, onView }: { project: Project; onDelete: () => void; onView: () => void }) {
...
export function ProjectsPage() {
    ...
    return (
      <div className="flex h-full flex-col">
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <ProjectDashboard projectId={viewingProject} onBack={() => setViewingProject(null)} />
        </div>
      </div>
    );
  }
  ...
  if (!backendOnline) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 text-center px-8 animate-fade-in">
        <div
          className="flex size-16 items-center justify-center rounded-2xl bg-[--accent-dim]/30"
        >
          <FolderOpen size={28} className="text-[--accent-hover]" strokeWidth={1.5} />
        </div>
        <div>
  ...
  return (
    <div className="flex h-full flex-col animate-fade-in">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[--border-subtle] header-gradient">
        <div>
          <h1 className="text-lg font-bold gradient-text">我的项目</h1>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        {loading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-6 text-center max-w-md mx-auto animate-fade-in">
            <div
              className="flex size-20 items-center justify-center rounded-2xl bg-[--accent-dim]/30"
            >
              <Film size={36} className="text-[--accent-hover]" strokeWidth={1.5} />
            </div>
            <div>
          ...
          <div className="grid gap-4 grid-cols-[repeat(auto-fill,minmax(200px,1fr))]">
            {projects.map((project) => (
              <ProjectCard
                ...
                onView={() => setViewingProject(project.id)}
              />
            ))}
          </div>
        )}
