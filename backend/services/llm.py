...
def _get_active_llm() -> tuple[str, dict]:
    ...
    for provider in ("claude", "openai", "deepseek", "qianwen", "glm", "minimax", "ollama"):
...
class LLMService:
    ...
    async def stream(self, prompt: str, system: str = "", provider: str = None) -> AsyncIterator[str]:
        """Stream LLM response in text chunks. If provider is None or 'auto', auto-select."""
        if provider is None or provider == "auto":
            provider, cfg = _get_active_llm()
        else:
            cfg = _get_config(provider)
            if not cfg.get("api_key") and not (provider == "ollama" and cfg.get("base_url")):
                raise RuntimeError(f"未配置 {provider} 接口，请在设置页面配置")
        ...
        if provider == "claude":
            ...
            # OpenAI-compat (openai/deepseek/qianwen/glm/minimax) support streaming
            if provider == "qianwen" and not cfg.get("base_url"):
                cfg["base_url"] = "https://dashscope.aliyuncs.com/compatible-mode/v1"
            if provider == "qianwen" and not cfg.get("model"):
                cfg["model"] = "qwen-max"
            if provider == "glm" and not cfg.get("base_url"):
                cfg["base_url"] = "https://open.bigmodel.cn/api/paas/v4"
            if provider == "minimax" and not cfg.get("base_url"):
                cfg["base_url"] = "https://api.minimax.chat/v1"
    ...
    async def complete_with_provider(self, provider: str, prompt: str, system: str = "") -> str:
        # Handle "auto" by trying providers in order, falling back on failure
        if provider == "auto":
            providers_order = ("glm", "qianwen", "deepseek", "openai", "claude", "minimax", "ollama")
            for p in providers_order:
                cfg = _get_config(p)
                if cfg.get("api_key") or (p == "ollama" and cfg.get("base_url")):
                    try:
                        if p == "claude":
                            return await self._claude(prompt, system, cfg)
                        elif p in ("openai", "deepseek", "qianwen", "glm", "minimax"):
                            if p == "qianwen" and not cfg.get("base_url"):
                                cfg["base_url"] = "https://dashscope.aliyuncs.com/compatible-mode/v1"
                            if p == "glm" and not cfg.get("base_url"):
                                cfg["base_url"] = "https://open.bigmodel.cn/api/paas/v4"
                            if p == "minimax" and not cfg.get("base_url"):
                                cfg["base_url"] = "https://api.minimax.chat/v1"
                            return await self._openai_compat(prompt, system, cfg)
                        elif p == "ollama":
                            return await self._ollama(prompt, system, cfg)
                    except Exception as e:
                        print(f"[LLM] Provider {p} failed: {e}, trying next...")
                        continue
            raise RuntimeError("所有已配置的 LLM 接口均调用失败，请检查API密钥是否有效")

        # Specific provider requested
        cfg = _get_config(provider)
        if provider == "claude":
            return await self._claude(prompt, system, cfg)
        elif provider in ("openai", "deepseek", "qianwen", "glm", "minimax"):
            # Set default base_url for Chinese providers
            if provider == "qianwen" and not cfg.get("base_url"):
                cfg["base_url"] = "https://dashscope.aliyuncs.com/compatible-mode/v1"
            if provider == "glm" and not cfg.get("base_url"):
                cfg["base_url"] = "https://open.bigmodel.cn/api/paas/v4"
