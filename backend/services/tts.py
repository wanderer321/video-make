...
class TTSService:
    async def synthesize(
        ...
        print(f"[TTS] provider={provider}, voice={voice}, text_len={len(text)}, instructions_len={len(instructions)}")
        ...
        if provider == "edge_tts":
            ...
            return await self._qwen3_tts(text, voice, instructions)
        else:
            return await self._edge_tts(text, voice, speed)

    def _pick_provider(self) -> str:
        for p in ("qwen3_tts", "elevenlabs", "fish_audio", "azure_tts", "cosyvoice", "xunfei"):
            cfg = _get_config(p)
    ...
    async def _qwen3_tts(self, text: str, voice: str, instructions: str = "") -> bytes:
        """Qwen3-TTS-Instruct-Flash via DashScope MultiModalConversation API."""
        import os
        import dashscope
        cfg = _get_config("qwen3_tts")
        api_key = cfg.get("api_key") or os.getenv("DASHSCOPE_API_KEY", "")
        if not api_key:
            raise RuntimeError("Qwen3-TTS: DASHSCOPE_API_KEY not configured")
        dashscope.api_key = api_key
        voice_name = voice if voice in self.QWEN3_VALID_VOICES else "Cherry"
        call_kwargs = dict(
        ...
        if response.status_code != 200:
            raise RuntimeError(f"Qwen3-TTS failed: {response.message}")
        audio_obj = response.output.get("audio")
        if not audio_obj:
            raise RuntimeError("Qwen3-TTS: no audio in response")
        ...
        raise RuntimeError("Qwen3-TTS: no audio data or url in response")
