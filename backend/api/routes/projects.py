...
def _to_out(p: Project) -> dict:
    return {
        ...
        "workflow_step": p.workflow_step or 1,
        "breakdown_result": p.breakdown_result,
        "asset_prompts_map": p.asset_prompts_map,
...
def get_project(project_id: str, db: Session = Depends(get_session)):
    ...
    for ep in sorted(p.episodes, key=lambda e: e.episode_no):
        ...
        # Count images from shots, videos from boards
        image_count = 0
        for b in boards:
            for shot in b.shots:
...
class AssetPromptsMapUpdate(BaseModel):
    asset_prompts_map: dict


@router.patch("/{project_id}/asset-prompts")
def update_asset_prompts_map(project_id: str, body: AssetPromptsMapUpdate, db: Session = Depends(get_session)):
    """Save the prompts and assetId mapping for a project."""
    p = db.query(Project).filter(Project.id == project_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Project not found")
    p.asset_prompts_map = body.asset_prompts_map
    db.commit()
    return {"ok": True}
...
@router.patch("/{project_id}/episodes/{episode_id}/script")
def update_episode_script(project_id: str, episode_id: str, body: ScriptContentUpdate, db: Session = Depends(get_session)):
    """Save script content for an episode."""
    ep = db.query(Episode).filter(Episode.id == episode_id, Episode.project_id == project_id).first()
    if not ep:
        raise HTTPException(status_code=404, detail="Episode not found")
    ep.script_content = body.script_content
    db.commit()
    return {"ok": True}
...
class BreakdownResultUpdate(BaseModel):
    breakdown_result: dict


@router.patch("/{project_id}/breakdown-result")
def update_project_breakdown(project_id: str, body: BreakdownResultUpdate, db: Session = Depends(get_session)):
    p = db.query(Project).filter(Project.id == project_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Project not found")
    p.breakdown_result = body.breakdown_result
    db.commit()
    return {"ok": True}
